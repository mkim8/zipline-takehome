...
class Main
  ...
  def self.determine_first_seen_row_index_for_keys_in_row(keys_header_indices, row, email_or_phone_number_type,
                                                          row_index, union_find, first_row_seen_for_key)
    keys_header_indices.each do |i|
      next if row[i].nil?

      key = row[i].to_s.strip.downcase

      key = get_phone_number_from_string(key) if email_or_phone_number_type == MATCH_TYPE[:phone_number]
    ...
    CSV.foreach(file_path).with_index do |row, i|
      if i.zero?
        ...
        when MATCH_TYPE[:email_address]
          union_find = determine_first_seen_row_index_for_keys_in_row(email_header_indices, row,
                                                                      MATCH_TYPE[:email_address], i, union_find, first_row_seen_for_key)
        when MATCH_TYPE[:phone_number]
          union_find = determine_first_seen_row_index_for_keys_in_row(phone_number_header_indices, row,
                                                                      MATCH_TYPE[:phone_number], i, union_find, first_row_seen_for_key)
